...
def initial_state():
    ...
    # *** START CODE HERE ***
    return {} # dictionary to be filled with betas and x's
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    sum = 0
    for i in state.keys():
        sum += state[i][0] * kernel(state[i][1], x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    kernel_sum = 0
    for i in state.keys():
        kernel_sum += state[i][0] * kernel(state[i][1], x_i)

    beta = learning_rate * (y_i - sign(kernel_sum))
    state[len(state.keys())] = (beta, x_i)  # add new (beta, x) pair to dictionary
    # *** END CODE HERE ***
